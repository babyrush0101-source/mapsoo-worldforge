...
func _init() -> void:
	...
	if manifest.get("schema_version") != "0.8.0":
		_fail("Pack 0.8 fixture manifest is missing or invalid: %s" % manifest_path)
		return
	...
	if result.get("ok", false) != true or first_status not in ["created", "updated", "unchanged"]:
		_fail("First Pack 0.8 import failed: %s" % result)
		return
	...
	if repeated.get("ok", false) != true or repeated.get("status", "") != "unchanged":
		_fail("Second Pack 0.8 import must be unchanged: %s" % repeated)
		return
	print("MAPSOO_ALPHA11_GODOT_OK pack_id=%s schema=0.8.0 first=%s second=unchanged roles=36 characters=3 animations=128 ysort=true navigation=true" % [manifest.pack.id, first_status])
	quit(0)


func _world_error(world: Node) -> String:
	if world.name != "MapsooWorld" or world.get_meta("mapsoo_profile", "") != "isometric-action" or world.get_meta("mapsoo_schema_version", "") != "0.8.0":
		return "Pack 0.8 root metadata is invalid."
	var ysorted := world.get_node_or_null("YSortedGameplay") as Node2D
	if ysorted == null or not ysorted.y_sort_enabled:
		return "Pack 0.8 Y-sorted gameplay domain is missing."
	if not (world.get_node_or_null("Floor") is Node2D) or world.get_node("Floor").get_child_count() != 62:
		return "Pack 0.8 floor cells are incomplete."
	if not (world.get_node_or_null("Elevation") is Node2D) or world.get_node("Elevation").get_child_count() != 2:
		return "Pack 0.8 elevated cells are incomplete."
	var player := world.get_node_or_null("YSortedGameplay/Actors/Player") as CharacterBody2D
	var melee := world.get_node_or_null("YSortedGameplay/Actors/EnemyMelee") as CharacterBody2D
	var ranged := world.get_node_or_null("YSortedGameplay/Actors/EnemyRanged") as CharacterBody2D
	if player == null or player.get_script() != PlayerController or melee == null or ranged == null:
		return "Pack 0.8 three-character hierarchy is incomplete."
	for actor: CharacterBody2D in [player, melee, ranged]:
		var visual := actor.get_node_or_null("Visual") as AnimatedSprite2D
		if visual == null or visual.sprite_frames == null:
			return "Pack 0.8 actor visual is missing."
		var actions := PLAYER_ACTIONS if actor == player else ENEMY_ACTIONS
		for action: String in actions:
			for direction: String in DIRECTIONS:
				...
				if not visual.sprite_frames.has_animation(clip) or visual.sprite_frames.get_frame_count(clip) != 1:
					return "Pack 0.8 actor is missing clip %s." % clip
				if not _texture_has_persisted_pixels(visual.sprite_frames.get_frame_texture(clip, 0)):
					return "Pack 0.8 actor clip lost persisted pixels: %s." % clip
	...
	if navigation == null or navigation.navigation_polygon == null or navigation.navigation_polygon.get_polygon_count() < 1:
		return "Pack 0.8 navigation polygon is incomplete."
	var traversal := world.get_node_or_null("WorldTraversal") as Node2D
	if traversal == null or traversal.get_child_count() != 5 or traversal.get_meta("mapsoo_exit_node_id", "") != "exit-node":
		return "Pack 0.8 traversal graph is incomplete."
	if not (world.get_node_or_null("WorldCollision") is Node2D) or world.get_node("WorldCollision").get_child_count() != 2:
		return "Pack 0.8 blockers are incomplete."
	if not (world.get_node_or_null("Hazards") is Node2D) or world.get_node("Hazards").get_child_count() != 1:
		return "Pack 0.8 hazards are incomplete."
	return ""
...
func _fail(message: String) -> void:
	push_error("MAPSOO_ALPHA11_GODOT_FAILURE: %s" % message)
	quit(1)
