import type { BuildWorldArtRuntimeOverlayV1_1Input } from './build-world-art-runtime-overlay-v1-1';
...
export const OVERLAY_V1_1_PRIVATE_MARKER =
  'PRIVATE_OVERLAY_1_1_DO_NOT_EXPORT';

const FACTS: ConfirmedWorldFacts = Object.freeze({
  premise: `${OVERLAY_V1_1_PRIVATE_MARKER} traveler restores a route.`,
  worldview: `${OVERLAY_V1_1_PRIVATE_MARKER} paths change cooperation.`,
  terrain: `${OVERLAY_V1_1_PRIVATE_MARKER} readable ridges, water, terraces, and gardens.`,
  geography: `${OVERLAY_V1_1_PRIVATE_MARKER} gate, market, workshop, homes, and beacon.`,
  culture: `${OVERLAY_V1_1_PRIVATE_MARKER} crafts, lanterns, meals, and performances.`,
  ecology: `${OVERLAY_V1_1_PRIVATE_MARKER} trees, reeds, flowers, birds, moss, and pools.`,
  mood: `${OVERLAY_V1_1_PRIVATE_MARKER} hopeful exploration with gentle mystery.`,
  art_direction: `${OVERLAY_V1_1_PRIVATE_MARKER} original pixel art and clear silhouettes.`,
  traversal: `${OVERLAY_V1_1_PRIVATE_MARKER} start, cross, visit landmarks, and exit.`,
  landmarks: `${OVERLAY_V1_1_PRIVATE_MARKER} gate, market, workshop, beacon`,
});
...
export async function overlayV1_1Sha256Bytes(
  bytes: Uint8Array,
): Promise<string> {
...
async function withMapId(
  ...
  return materializeWorldArtPlacementMapEnvelope({
    ...value,
    map_id: `world-art-placement-map-${(
      await overlayV1_1Sha256Bytes(payload)
    ).slice(0, 16)}`,
  });
}

export async function buildWorldArtRuntimeOverlayV1_1TestFixture(
  ...
  const pngSha256 = await overlayV1_1Sha256Bytes(png);
  ...
  const projection = await buildWorldArtRuntimeProjection({
    ...
    rights: { distribution: 'public', license: 'CC0-1.0' },
    images: [{
      task_id: taskId,
      path: imagePath,
