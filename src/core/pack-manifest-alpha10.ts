...
export function validateAlpha10PackManifest(manifest: Alpha10PackManifest): Alpha10RuntimeIssue[] {
  ...
  const referenced = [...manifest.atlases.map((x) => x.path), ...manifest.roles.map((x) => x.path), manifest.character.atlas, manifest.runtime.scene.path, manifest.runtime.collision.path, manifest.runtime.navigation.path, manifest.license.output.notice_path, ...(manifest.layout ? [manifest.layout.path] : []), ...(manifest.material_palette ? [manifest.material_palette.path] : []), ...(manifest.terrain_autotiles ? [manifest.terrain_autotiles.path] : [])];
